...
class RandomLinkSplit(BaseTransform):
    ...
    def __call__(self, data: Union[Data, HeteroData]):
        ...
        for item in zip(stores, train_stores, val_stores, test_stores,
            ...
            num_val_nodes = self.num_val
            if isinstance(num_val_nodes, float):
                num_val_nodes = int(num_val_nodes * perm.numel())
            num_test_nodes = self.num_test
            if isinstance(num_test_nodes, float):
                num_test_nodes = int(num_test_nodes * perm.numel())

            num_train_nodes = perm.numel() - num_val_nodes - num_test_nodes
            if num_train_nodes <= 0:
                raise ValueError("Insufficient number of edges for training")

            train_nodes = perm[:num_train_nodes]
            ...
            if self.add_negative_train_samples:
                if num_disjoint > 0:
                    ...
                    num_neg_train = int(num_train_edges * self.neg_sampling_ratio)
            num_neg_val = int(num_val_edges * self.neg_sampling_ratio)
            num_neg_test = int(num_test_edges * self.neg_sampling_ratio)
