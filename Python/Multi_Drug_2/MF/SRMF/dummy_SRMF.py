...
def algorithm_update(U, V, W, R, S, lambda_l, lambda_d):
    X = (np.matmul(R,V)) + (2*lambda_d*(np.matmul(S,U)))
    Y = 2*lambda_d*(np.matmul(U.T,U))
...
sen = np.matrix('1,2,3,4,5;1,2,1,2,1;3,6,7,8,9')
#sen = sen.T
sen = np.array(sen)
...
Fingerprints_sim = np.matrix('1,2,3;1,2,1;3,6,7')
Fingerprints_sim = np.array(Fingerprints_sim)

#Sample_Sim_Exp = pd.read_csv("Data/Sample_Sim_Exp.csv", header = 0, index_col=0, sep = ',' )
Sample_Sim_Exp = np.matrix('1,2,3,4,5;1,2,1,2,1;3,6,7,8,9;1,2,2,2,2;8,8,8,6,5')
